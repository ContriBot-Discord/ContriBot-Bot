...
export class UserItem {
  ...
  delete() {
    this.#db.execute("DELETE FROM INVENTORY WHERE id = ?", [this.id], (err) => {
      if (err) throw err;
    });
  }

  toBoost(): Boost {
    // Convertir la durée du boost en millisecondes
    const boostDurationParts = this.boostDuration!.split(":");
    const boostDurationMilliseconds =
      (parseInt(boostDurationParts[0]) * 60 * 60 +
        parseInt(boostDurationParts[1]) * 60 +
        parseInt(boostDurationParts[2])) *
      1000;

    // Calculer la date d'expiration du boost en ajoutant la durée convertie à la date d'achat
    const expirationDate = new Date(
      this.purchaseDate.getTime() + boostDurationMilliseconds
    );
