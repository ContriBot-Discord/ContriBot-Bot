...
const event: BotEvent = {
    ...
    async execute(oldState: VoiceState, newState: VoiceState) {
        // TODO: Use a real method to add voice xp. XP is not added yet.
        ...
        // We are checking if the user is "eligible" for voice xp
            if ( !newState.deaf && !newState.mute && ! (newState.channelId === null)  ) return;
                // In that case, the user is eligible for voice xp
        ...
        // That means the user is NOT muted, NOT deafened and is in a voice channel
            ...
            if (
                ...
                user.addVoiceXp(voiceDuration, oldState.channel!.id, oldState.guild.id, oldState.member!.roles.cache.map(role => role.id));
                    ...
                    newState.guild.id === oldState.guild.id && newState.channelId !== oldState.channelId
                ){
